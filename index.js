...
function checkEmail(str) {
  return /^[a-zA-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{1,3}$/.test(str);
}
...
function checkDomainUrl(str) {
  return /^(http:|https:)\/\/([a-zA-Z0-9-_.]+(\.[a-zA-Z0-9-_.]+)*\.[a-zA-Z]{2,})$/.test(
    str
  );
}
...
function createLinksFromDomains(str) {
  const regex =
    /(http:|https:)\/\/(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}/gi;
  ...
  matches.forEach((match) => {
    const domain = match.replace(/^(http:|https:)\/\//gi, '');
    const link = `<a href="${match}" target="_blank">${domain}</a>`;
